...
export default function ChooseShipping() {
  ...
  const homeCoordinate = () => {
    Axios.get(
      `${process.env.REACT_APP_API_BASE_URL}/address/address-city-id/${homeId}`
    ).then((res) => {
    ...
    for (let x = 0; x < allWH.length; x++) {
      mathDist.push(
        getDistance(
          parseInt(homeLat),
          parseInt(homeLon),
          parseInt(allWH[x].latitude),
          parseInt(allWH[x].longitude)
        )
      );
  ...
  const ongkirCount = () => {
    ...
    Axios.post(
      `${process.env.REACT_APP_API_BASE_URL}/address/ongkir-type`,
      req
    ).then((res) => {
  ...
  const getOrder = () => {
    Axios.get(
      `${process.env.REACT_APP_API_BASE_URL}/order/get-order/${user?.customer_uid}`
    )
      .then((result) => {
  ...
  const courierSelect = (
    <div className="ship-courier-selector">
      <select
        ...
        onClick={homeCoordinate}
      >
        <option>JNE</option>
        <option>POS Indonesia</option>
        <option>TIKI</option>
      </select>
      <button className="courier-acc" onClick={ongkirCount}>
        Select Courier
      </button>
    </div>
  );
      ...
      <div className="ship-choice">
        <div className="ship-method">
          ...
          <div className="ship-choice-text">
            ...
            <div className="ship-choice-spec">
              Estimated Arrival {x.cost[0].etd}
            </div>
          </div>
        </div>
        <div className="ship-select-dot">
          ...
          <div className="ship-selector">
            <input
              type="radio"
              value={x.cost[0].value}
              className="ship-radio"
              name="shipradio"
              onChange={(e) => setCostSelect(parseInt(e.target.value))}
            />
          </div>
        </div>
      </div>
  ...
  const ongkirFinal = () => {
    ...
    Axios.put(
      `${process.env.REACT_APP_API_BASE_URL}/order/edit-shipping/${user.customer_uid}`,
      data
    )
      .then(() => {
      ...
      <Container className="ship-container" maxWidth="xs">
        ...
        <div className="ship-subwrap">
          ...
          <div className="ship-continue">
            <button className="ship-button" onClick={ongkirFinal}>
              Apply
            </button>
          </div>
        </div>
